...
# t-SNE of RBP binding profiles with hierarchical clustering (Ward.D2, Gower distance).
#
# Input:
...
#
# Output:
# - outputs/rbp/figures/rbp_tsne_hclust_clusters.svg
...
out_svg <- file.path(out_dir, "rbp_tsne_hclust_clusters.svg")
...
# Hierarchical clustering on Jaccard distance (binary profiles).
diss <- dist(rbp_mat, method = "binary")
hc <- hclust(diss, method = "ward.D2")
cluster_assignments <- cutree(hc, k = k)
rbp_mat$Cluster <- as.factor(cluster_assignments)

# t-SNE on unique rows.
...
p_tsne <- ggplot(tsne_df, aes(x = tSNE1, y = tSNE2)) +
  ...
  labs(
    title = "t-SNE of RBP binding profiles (hierarchical clustering)",
    subtitle = paste0("Ward.D2 on Gower distance (k = ", k, ") | point color = bHLH class"),
    x = "t-SNE 1",
    y = "t-SNE 2",
    color = "bHLH class",
    shape = "Cluster"
