...
# t-SNE of RBP binding profiles with PAM clustering (Gower distance).
#
# Input:
...
#
# Output:
# - outputs/rbp/figures/rbp_tsne_pam_clusters.svg

project_root <- Sys.getenv("BHLH_PROJECT_ROOT", unset = ".")
source(file.path(project_root, "scripts", "lib", "bhlh_utils.R"))

require_pkgs(c("cluster", "Rtsne", "ggplot2", "dplyr", "ggrepel", "tibble", "viridis"))
...
out_svg <- file.path(out_dir, "rbp_tsne_pam_clusters.svg")
...
# PAM clustering on Jaccard distance (binary profiles).
diss <- dist(rbp_mat, method = "binary")
pam_res <- pam(diss, k = k, diss = TRUE)
rbp_mat$Cluster <- as.factor(pam_res$clustering)

# t-SNE on unique rows (Rtsne cannot handle duplicates by default).
...
# Hulls and centroids are computed per cluster (visual grouping), while point color encodes class.
...
p_tsne <- ggplot(tsne_df, aes(x = tSNE1, y = tSNE2)) +
  ...
  geom_point(aes(color = Ledent2002.Simionato2007), size = 2.5, alpha = 0.85) +
  ...
  labs(
    title = "t-SNE of RBP binding profiles (PAM clustering)",
    subtitle = paste0("PAM on Gower distance (k = ", k, ") | point color = bHLH class"),
    x = "t-SNE 1",
    y = "t-SNE 2",
    color = "bHLH class"
  ) +
