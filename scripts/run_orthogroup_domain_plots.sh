...
SKIP_EXISTING="${BHLH_SKIP_EXISTING:-1}"
BHLH_SKIP_EXISTING="${SKIP_EXISTING}" Rscript "${PROJECT_ROOT}/scripts/ortho_bHLH.R"

LEGACY_DIR="${PROJECT_ROOT}/outputs/orthogroups/domain_positions"
if [[ -d "${LEGACY_DIR}" ]]; then
  echo
  echo "Note: legacy output folder detected:"
  echo "  - ${LEGACY_DIR}/"
  echo "These SVGs were generated by an older plotting script and may show species IDs with underscores."
  echo "Use the new folders printed below instead."
fi
